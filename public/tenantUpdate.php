...
// tenant.flags:                (optional) The flags for the website.  Used on website.
// tenant.reseller_id:          (optional) The reseller for this website.
...
// <rsp stat='ok' />
//
function ciniki_sysadmin_tenantUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'tenant.flags'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Options'), 
        'tenant.sitename'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Sitename'), 
        'tenant.tagline'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Tagline'), 
        'tenant.reseller_id'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Reseller'), 
        ));
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['tenant.flags']) ) {
        $strsql .= ", flags = '" . ciniki_core_dbQuote($ciniki, $args['tenant.flags']) . "'";
        ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.tenants', 'ciniki_tenant_history', $args['tnid'], 
            2, 'ciniki_tenants', '', 'flags', $args['tenant.flags']);
    }
    ...
    if( isset($args['tenant.reseller_id']) ) {
        $strsql .= ", reseller_id = '" . ciniki_core_dbQuote($ciniki, $args['tenant.reseller_id']) . "'";
        ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.tenants', 'ciniki_tenant_history', $args['tnid'], 
            2, 'ciniki_tenants', '', 'reseller_id', $args['tenant.reseller_id']);
    }
    //
